...
class TurtleClient(Node):
    def __init__(self):
        ...
        #### Action Client ####
        self.action_cli = ActionClient(self, TurtleToGoals, 'go_to_goals')
        while not self.action_cli.wait_for_server(timeout_sec=1.0):
            self.get_logger().info('TravelToGoals action not available, waiting...')
        ########
    ...
    def turtle_callback(self,msg):
        ...
        self.turtle_display.setpos(msg.turtle_pose.position.x,msg.turtle_pose.position.y)

    def goal_actioncall(self):
        goal_msg = TurtleToGoals.Goal()
        for i in range(5):
            goal = Pose()
            goal.position.x = float(random.randint(int(-1*self.screen.window_width()/2)+20,int(self.screen.window_width()/2)-20))
            goal.position.y = float(random.randint(int(-1*self.screen.window_height()/2)+20,int(self.screen.window_height()/2)-20))
