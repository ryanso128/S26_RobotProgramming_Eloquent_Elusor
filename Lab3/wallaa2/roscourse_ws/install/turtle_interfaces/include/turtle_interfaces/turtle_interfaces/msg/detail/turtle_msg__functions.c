  ...
  turtle_interfaces__msg__TurtleMsg__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    turtle_interfaces__msg__TurtleMsg * data =
      (turtle_interfaces__msg__TurtleMsg *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!turtle_interfaces__msg__TurtleMsg__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          turtle_interfaces__msg__TurtleMsg__fini(&output->data[i]);
        }
        return false;
      }
    }
