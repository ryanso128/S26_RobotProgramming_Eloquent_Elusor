  ...
  turtle_interfaces__srv__SetPose_Request__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    turtle_interfaces__srv__SetPose_Request * data =
      (turtle_interfaces__srv__SetPose_Request *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!turtle_interfaces__srv__SetPose_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          turtle_interfaces__srv__SetPose_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
  ...
  turtle_interfaces__srv__SetPose_Response__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    turtle_interfaces__srv__SetPose_Response * data =
      (turtle_interfaces__srv__SetPose_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    ...
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!turtle_interfaces__srv__SetPose_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          turtle_interfaces__srv__SetPose_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
