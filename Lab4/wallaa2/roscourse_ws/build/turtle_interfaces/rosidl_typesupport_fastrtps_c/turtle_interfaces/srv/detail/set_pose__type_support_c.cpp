  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = turtle_interfaces__srv__SetPose_Request;
    is_plain =
      (
      offsetof(DataType, turtle_pose) +
      last_member_size
      ) == ret_val;
  }
...
static size_t _SetPose_Request__max_serialized_size(char & bounds_info)
{
  ...
  ret_val = max_serialized_size_turtle_interfaces__srv__SetPose_Request(
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    // We still need to check that the in-memory alignment
    // is the same as the CDR mandated alignment.
    using DataType = turtle_interfaces__srv__SetPose_Response;
    is_plain =
      (
...
static size_t _SetPose_Response__max_serialized_size(char & bounds_info)
{
  ...
  ret_val = max_serialized_size_turtle_interfaces__srv__SetPose_Response(
    full_bounded, is_plain, 0);
