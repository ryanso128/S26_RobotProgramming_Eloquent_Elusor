...
class TurtlebotServer(Node):
    def __init__(self):
        ...
        self.vel_x = 0 # velocity in x-direction (in the turtle frame), unit: pix/sec
        self.vel_y = 0 # velocity in y-direction (in the turtle frame), unit: pix/sec
        self.ang_vel = 0 # angular velocity in yaw-direction (in the turtle frame), unit: rad/sec
    ...
    def driving_timer_cb(self):
        ...
        new_x = self.turtle.turtle_pose.position.x + self.vel_x*self.sim_interval*math.cos(yaw) - self.vel_y*self.sim_interval*math.sin(yaw)
        new_y = self.turtle.turtle_pose.position.y + self.vel_x*self.sim_interval*math.sin(yaw) + self.vel_y*self.sim_interval*math.cos(yaw)
